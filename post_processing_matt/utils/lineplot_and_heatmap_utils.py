...
def make_plot_and_heatmap(photometry_data, *mean_data, error_bar_method='sem', mean_across_mice=False, xlims=[-2, 2], cue_vline=0, psycho=0):
    ...
    if mean_across_mice:
        ...
        fig.tight_layout(pad=3.5)    # pad = 2.1 original by MW
    ...
    if psycho == 0:
        ...
        heatmap_min, heatmap_max = make_y_lims_same((photometry_data.trials_98.min, photometry_data.trials_98.max),
                                                (photometry_data.trials_2.min, photometry_data.trials_2.max))
    ...
    if psycho == 0:
        ...
        ipsi_heatmap = plot_one_side(photometry_data.trials_98, fig, axs[1, 0], axs[1, 1], dff_range,
                             error_bar_method=error_bar_method, cue_vline=cue_vline)
        contra_heatmap = plot_one_side(photometry_data.trials_2, fig, axs[0, 0], axs[0, 1], dff_range,
                             error_bar_method=error_bar_method, cue_vline=cue_vline)
    ...
    if psycho == 0:
        axs[0, 0].set_ylabel('contralateral\nz-score')
        axs[1, 0].set_ylabel('ipsilateral\nz-score')
    elif psycho == 1:
        axs[0, 0].set_ylabel('2%\nz-score')
        axs[1, 0].set_ylabel('98%\nz-score')
    ...
    if mean_across_mice:
        ...
        for ax in [axs[0, 0], axs[1, 0]]:
            adjust_label_distances(ax, x_space=0.1, y_space=0.12)
